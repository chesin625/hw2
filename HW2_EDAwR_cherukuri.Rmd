...
https://github.com/chesin625/hw2
...
> Each row in the county_df gives us the census information (race, sex, income, poverty, commute type, employment) for each county/state. While rows in county_MI_df give us census information for each county in Michigan for upper peninsula and lower peninsula.
...
sum(is.na(county_df))
sum(is.na(county_mi_df))
```

> I used sum(is.na(county_df)) to find out if there are any na values in the two data frames. county_df had 1 and county_mi had 0.
The difference between the two data franes is, data has been further divided based on the peninsula in county_mi_df.
...
mean_units <- mean(county_df$Income)
mean_units
median(county_df$Income)
sd_units <- sd(county_df$Income)
sd_units
min(county_df$Income)
max(county_df$Income)
range_units <- max(county_df$Income) - min(county_df$Income)
quantile(county_df$Income, 0.95)
quantile(county_df$Income, 0.99)
quantile(county_df$Income, 0.75) - quantile(county_df$Income, 0.25)
cv_units <- sd_units/mean_units
cv_units
```
```{r summary_stats for IncomePerCap}
mean1_units <- mean(county_df$IncomePerCap)
mean1_units
median(county_df$IncomePerCap)
sd1_units <- sd(county_df$IncomePerCap)
sd1_units
min(county_df$IncomePerCap)
max(county_df$IncomePerCap)
range1_units <- max(county_df$IncomePerCap) - min(county_df$IncomePerCap)
quantile(county_df$IncomePerCap, 0.95)
quantile(county_df$IncomePerCap, 0.99)
quantile(county_df$IncomePerCap, 0.75) - quantile(county_df$IncomePerCap, 0.25)
cv1_units <- sd_units/mean_units
cv1_units
```
...
ggplot(county_df, aes(x=log(Income))) +
  geom_histogram(aes(y =..density..)) +
  geom_density(alpha = .2, fill = "#FF6666")

ggplot(county_df, aes(x=IncomePerCap)) +
   geom_histogram(aes(y =..density..)) +
  geom_density(alpha = .2, fill = "#FF6666")
```
...
>From the histograms it can be seen that they are right skewed. 
THe peak IncomePerCap is around 22000-28000 and the data is spread between 12000 - 55000. I see some outliers above the value 60000 for IncomePerCap.
By adding kernel density plot it shows that it is a good fit.
By only plotting for Income it was showing exponential values. SO I also plotted a histogram for log(income) to compare the distribution and skewness.
...
> Your interpretation ...
> The boxplot for Income shows that there are quiet a few outliers outside the quarlies for all regions. The boxplots for midwest and west seem to be symmetric.
While the boxplots for northeast and south seem to be right skewed.
In the boxplot for IncomePerCap there are more outliers for midwest compared to the previous box plot. The boxplot for midwest is symmetric. 
Box plots for northeast, south and west seem to be right skewed. The median range for incomepercap based on the 4 boxplots is 25000 - 30000.
From the violin plot for Incomepercap it can be seen that a wider number of population fall under the income range of 21000 - 28000. The probability is low for population to have an income above 45000.
...
```{r histo_lt1.5m1}
ggplot(county_df[county_df$TotalPop  < 500000,]) +
  geom_histogram(aes(x=log(TotalPop)),
                 fill = "lightblue", colour = "red") + 
                labs (title = "Income for counties with less than 500,000 people" , x = "Total Population", y = "Number of counties")
```
...
ggplot(county_mi_df) +
  geom_violin(aes(x = peninsula, y = Income),
              fill="lightblue", colour="red") +
  labs(title = "Distribution of Median Income in Michigan Peninsula's", x = "Michigan Peninsula", y = "Mean Income")
```
...
> Your discussion...
The income is spread wider for both the peninsulas. For lp the median income is near 45000. While the median income for up is around 43000.
The income for up does not go above 48000. For lp the income is spread upto 78000.
...
```{r boxplots_upvlp}
ggplot(county_mi_df) +
  geom_violin(aes(x = peninsula, y = MeanCommute), fill="lightblue", colour="red") +
   labs(title = "Mean Commute in Michigan Peninsula's", x = "Michigan Peninsula", y = "Mean Commute")

ggplot(county_mi_df) +
  geom_violin(aes(x = peninsula, y = TotalPop), fill="lightblue", colour="red") +
   labs(title = "Total Population in Michigan Peninsula's", x = "Michigan Peninsula", y = "Total Population")

ggplot(county_mi_df) +
  geom_boxplot(aes(x = peninsula, y = Unemployment),
               fill="lightblue", colour="black") +
   labs(title = "Unemployment in Michigan Peninsula's", x = "Michigan Peninsula", y = "Unemployment Rate")

ggplot(county_mi_df) +
  geom_boxplot(aes(x = peninsula, y = Employed),
               fill="lightblue", colour="black") +
   labs(title = "No. of people employed in Michigan Peninsula's", x = "Michigan Peninsula", y = "Employment Rate")
```

> Your discussion ...
The first violin plot shows the mean commute in both the peninsulas. The distribution is widely spread in both the peninsulas around the median.
While people commute anywhere between 16 - 34mins in the lp, commute time is between 10 to 24mins in up.
The second violin plot shows the total population in the peninsulas. In up the total population is wide spread or minimal between 0 - 15000.
The violin plot for lp shows that most of the population is clustered around 10000 but small amount of values are pulling the mean up 175000.
The box plots show the unemployment rate and employment rate in the peninsulas. The median unemployment rate for lp is around 7.5 and the box plot looks symmetric, but has some outliers. The median unemployment rate in up is around 7 and the plot is right skewed.The unemployment rate is higher in up than in lp.
The box plot for employment shows that the employment rate is low in up.
...
ggplot(county_df) +
  geom_bar(aes(x = State, y = County), stat = "identity") + 
  labs(title="Number of counties by State in the US", x="US States", y="County") +
...
ggplot(county_df, aes(x = reorder(factor(State), State, function(x) length(x)), y = County, fill = Region)) +
...
ggplot(county_mi_df) +
  geom_point(aes(x = IncomePerCap, y = MeanCommute, colour = peninsula, size = TotalPop))

ggplot(county_mi_df) +
  geom_point(aes(x = Income, y = Professional, colour = peninsula, size = TotalPop))
```

> Your thoughts on the relationship between these variables?
> There is no corelation between incomerpercap and mean commute. There are many outliers.
There is a positive correlation between income and professional. Though there are few outliers, we can see that the income increases based on profession.
...
ggplot(county_mi_df) +
  geom_point(aes(x = IncomePerCap, y = MeanCommute, colour = peninsula)) +
  facet_wrap(~peninsula)

ggplot(county_mi_df) +
  geom_point(aes(x = Income, y = Professional, colour = peninsula)) +
  facet_wrap(~peninsula)

ggplot(county_mi_df) +
  geom_point(aes(x = TotalPop, y = Unemployment, colour = peninsula)) +
facet_wrap(~peninsula)
```

> Discussion
Since the population is less in the upper peninsula compared to the lower peninsula, the unemployment rate is high in up comapred to lp.
There is no corelation between IncomPerCap and Mean commute as there is no uphill or downhill pattern. It is more spreadout, many outliers.
There is a positive corelation between Income and profesional. As the income increase by profesion.
...
```{r log10}
ggplot(county_df) +
  geom_point(aes(x = TotalPop, y = IncomePerCap, colour = Region)) +
  xlab("log(TotalPop)") + scale_x_log10()
```

> Discussion ...
The TotalPop has been log transformed as it was hard to interpret patterns. After transformation to log, the data is still in cluster.
This shows there is no corelation between IncomePerCap and Log(TotalPop).
